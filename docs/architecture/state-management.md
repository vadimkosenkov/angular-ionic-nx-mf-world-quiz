...
| `SyncService`   | `client-progress` | status (`idle` / `syncing` / `offline`), last sync, the API's answers this run | `resultOf(sessionId)` (a challenge's verdict)                                                                                                                | none (drives `ProgressStore`)                                                                |
