...
| `shared-contracts`    | Session request schema: accepts a played session, is exactly a domain `SessionRecord` (compile-time), rejects unknown fields such as `score`, malformed answers and too many answers                                                                                                                                                                                                                                                                                                             | Unit                                     |
...
| `api`                 | Config (database selection, production guard, secrets never echoed); request hashing; repository on real SQL (insert/read, duplicate id writes nothing, cascade, database constraints); `/v1/sessions` over HTTP on in-memory PGlite with the real migrations: server grading, idempotent retry, key-order independence, 409 conflict, Endless stop, 422 for sessions that cannot be replayed or have implausible times, 400/413 for bad bodies, 404/400 on read; `/health`; 500 without leaking | Unit + API (supertest + PGlite)          |
