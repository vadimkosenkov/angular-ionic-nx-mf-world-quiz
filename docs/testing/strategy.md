...
| `client-quiz-ports`   | Default dataset and clock behind the shared tokens                                                                                                                                                                                                                                                                                                                         | Unit (TestBed)                           |
| `client-quiz-feature` | Session store (start, determinism, feedback gate, wrong answers, completion, typos, timed expiry, endless stop); play screen (prompt, choices, feedback, finished event, hard-mode validation); results screen (score, review list, perfect run, unlocked achievements, actions)                                                                                           | Unit + component                         |
...
| `capitals`            | The remote's page: query parameters → `QuizConfig`, defaults for unknown values, handing the finished session to the host, play again                                                                                                                                                                                                                                      | Component + integration                  |
| `flags`               | Root component renders                                                                                                                                                                                                                                                                                                                                                     | Component (placeholder)                  |
| `shell-e2e`           | Home with real totals; tab navigation; deep links and unknown routes; system vs explicit theme across reloads; switching to Russian and persistence; bundled flag assets; the Capitals quiz journey across the microfrontend boundary (setup → 10 questions → perfect result; a mistake recorded in the shell; leaving the quiz)                                           | E2E                                      |
...
(`nx run-many -t serve-static -p shell capitals`): the quiz only exists if the
