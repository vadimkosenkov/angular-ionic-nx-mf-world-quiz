...
- Google's own button, rendered by **Google Identity Services** (GIS), returns
  ...
  256 random bits (`crypto.getRandomValues`), new for every sign-in attempt:
  after each ID token the button is set up again with a fresh nonce.
...
- The access token lives in a private field of `AuthStore`; the refresh token
  ...
  like theft (reuse detection) and sign the player out. Across tabs, the
...
- Two refreshes racing with the same token look like reuse and sign the client
  out; clients must serialise refreshes — in a tab (shared promise) and
  across tabs (Web Lock). Browsers without Web Locks (none current) fall back
  to per-tab serialisation.
