...
- Idempotency: client-generated UUID `id` + SHA-256 of the canonical request (`request-hash.ts`). Same body → 200 with the stored result; different body → 409; races resolved by the primary key + `ON CONFLICT DO NOTHING`.
